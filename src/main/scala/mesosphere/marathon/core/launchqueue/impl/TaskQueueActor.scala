...
private[impl] object TaskQueueActor {
  def props(appActorProps: (AppDefinition, Int) => Props): Props = {
    Props(new TaskQueueActor(appActorProps))
  }
...
/**
  ...
  */
private[impl] class TaskQueueActor(appActorProps: (AppDefinition, Int) => Props) extends Actor with ActorLogging {
  ...
  private[this] def receiveHandlePurging: Receive = {
    case Purge(appId) =>
      launchers.get(appId).foreach { actorRef =>
